 ...
 */
public final class GClockProPolicy implements KeyOnlyPolicy {
  ...
  public GClockProPolicy(Config config) {
    ...
    this.policyStats = new PolicyStats("irr.GClockPro");
  ...
  public static Set<Policy> policies(Config config) {
    return ImmutableSet.of(new GClockProPolicy(config));
  }
  ...
  private void runHandCold() {
    ...
    if (handCold.marked()) {
      ...
      if (handCold.isInTest()) {
        ...
        handCold.removeFromClockPermanently();
      }
  ...
  private boolean runHandHot(Node trigger) {
    ...
    while (handHot != trigger) {
      if (handHot.isHot()) {
        ...
        // TODO: Decrease node's age. I divide it by 2 to achieve log (age) speed. I'm mildly worried
        // TODO: about it takes too long if decrease the counter by 1.
        handHot.age = handHot.age / 2;
        if (handHot.marked()) {
          handHot.moveToHead(Status.HOT);
        } else {
          if (handHot.age > 0) {
            // TODO: If node's age is bigger than 0, move to head with keeping hot status.
            handHot.moveToHead(Status.HOT);
          } else {
            // TODO: If node's age is 0, demote.
  ...
  private void terminateTestPeriod(Node node) {
    ...
    if (node.isResidentCold()) {
      ...
      node.removeFromClockPermanently();
    }
  ...
  final class Node {
    ...
    // TODO: I changed boolean variable "marked" to integer counter and counting the access time.
    // TODO: age is for summing marked from the node is created, until it is removed permanently.
    int marked;
    int age;
    ...
    public void removeFromClockPermanently() {
      removeFromClock();
      // TODO: reset the age when the node is permanently removed.
      age = 0;
    }
    ...
    public void removeFromClock() {
      ...
      // TODO: Add the number of accessed time to age.
      age += marked;
      // TODO: Then, reset the marked counter.
      marked = 0;
    }
