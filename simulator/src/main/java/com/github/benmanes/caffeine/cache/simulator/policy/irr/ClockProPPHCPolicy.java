 ...
 */
public final class ClockProPPHCPolicy implements KeyOnlyPolicy {
  ...
  public ClockProPPHCPolicy(Config config) {
    ...
    this.policyStats = new PolicyStats("irr.ClockProPPHC");
  ...
  public static Set<Policy> policies(Config config) {
    return ImmutableSet.of(new ClockProPPHCPolicy(config));
  }
  ...
  public void record(long key) {
    ...
    if ((policyStats.operationCount() % Math.min(maxSize, 10000)) == 0) {
      ...
      double hitRateChange = hitRateInSample - prevHitRate;

      double amount = (hitRateChange >= 0) ? nrStepSize : -nrStepSize;
      double nextNRStepSize = (Math.abs(hitRateChange) >= HILL_CLIMB_RESTART_THRESHOLD)
          ? HILL_CLIMB_NR_PERCENT * maxSize * (amount >= 0 ? 1 : -1)
          : HILL_CLIMB_NR_DECAY_RATE * amount;
      maxNonResSize = Math.max(minResColdSize, maxNonResSize + (int) nrStepSize);
      nrStepSize = nextNRStepSize;
      while (sizeNonResCold > maxNonResSize) {
        runHandTest();
      }
      // System.out.println(maxNonResSize / (double) maxSize);

      prevHitRate = hitRateInSample;
...
//    // This is the author's algorithm.
//    int targetHot = maxSize - coldTarget;
//    if (sizeHot < targetHot) {
//      // Promotion without demotion.
//      return true;
//    } else if (sizeHot > targetHot) {
//      // Demotion without promotion.
//      runHandHot(candidate);
//      return false;
//    }
//    // One demotion, one promotion.
